<div align="center">
  <h1>Hello, AIU Admissions Committee! 👋</h1>
  <h3>DevOps & Computer Engineering Student</h3>
</div>

<p align="center">
  <img src="https://i.pinimg.com/736x/73/0f/2c/730f2c2aaa92463b6bc31e5f5edb190d.jpg" 
       alt="Banner" 
       width="40%" 
       style="border-radius: 15px;"/>
</p>

<div align="center">
  <h2>Welcome to my Profile</h2>
</div>

---

## About Me

- **Name:** Emir Bekzhanov
- **Current Role:** DevOps Engineer at Alif Group
- **Education:** Computer Engineering @ Ala-Too International University
- **Interests:** System Architecture • Cloud Infrastructure • High-load Systems • DevOps • Automation

---

## Production web platform.
**My contribution**

- Docker deployment
- Linux server administration
- Nginx Reverse Proxy
- SSL configuration
- CI/CD pipelines
- Monitoring & maintenance

Workspace Direct — [workspace-direct.com](https://workspace-direct.com)  
Мультимодульная веб-платформа для управления рабочим пространством.  
**Стек:** Next.js, Django, Nginx, Docker.

Edu ERP — [edu-erp.today](https://edu-erp.today)  
Масштабируемая ERP-система для автоматизации образовательных процессов.

Tournament System — [tournament-system.digital](https://tournament-system.digital)  
Платформа для организации и проведения турниров.

 Asman Tourism — [asmantourism.info](https://asmantourism.info)  
Высоконагруженная веб-платформа для туристического сектора.


---

## Skills & Technologies

<p align="center">
  <img src="https://img.shields.io/badge/C%23-239120?logo=csharp&logoColor=white" />
  <img src="https://img.shields.io/badge/.NET-512BD4?logo=dotnet&logoColor=white" />
  <img src="https://img.shields.io/badge/Docker-2496ED?logo=docker&logoColor=white" />
  <img src="https://img.shields.io/badge/Linux-FCC624?logo=linux&logoColor=black" />
  <img src="https://img.shields.io/badge/Google_Cloud-4285F4?logo=googlecloud&logoColor=white" />
  <img src="https://img.shields.io/badge/GitHub_Actions-2088FF?logo=github-actions&logoColor=white" />
</p>

---

## Personal & Pet Projects

**₿ Crypto Simulator**  
Прототип криптовалютной платформы с акцентом на безопасность и архитектуру.  
 [github.com/miroku312/Crypto_simulator](https://github.com/miroku312/Crypto_simulator)

** Genshin Interactive Map**  
Интерактивная карта для Genshin Impact с маркерами и навигацией.  
 [github.com/kamitairova/GenshinMap](https://github.com/kamitairova/GenshinMap)

** Emir's Dormshade**  
Игра, разработанная на Unity в рамках личного проекта.  
 [github.com/elifdigitals/Emirs-Dormshade](https://github.com/elifdigitals/Emirs-Dormshade/tree/main/Emirs-Dormshade)

---

<div align="center">
  <img src="https://i.pinimg.com/736x/9c/c6/fa/9cc6fa047d2fcf47e3d9357a8f194984.jpg" 
       width="50%" 
       alt="Thank you"/>
</div>

<div align="center">
  <strong>Thank you for visiting my profile!</strong>
</div>
